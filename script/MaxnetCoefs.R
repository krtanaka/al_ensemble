MaxnetCoefs = function (model, maxnet2d = NULL) 
{
  ...
  if (length(maxnet2d) > 0) {
    ...
    for (i in 1:length(maxnet2d)) {
      coef2d1 <- unlist(lapply(X = beta.names, FUN = function(x, 
                                                              y) {
        y %in% x
      }, y = maxnet2d[[i]][1]))
      coef2d2 <- unlist(lapply(X = beta.names, FUN = function(x, 
                                                              y) {
        y %in% x
      }, y = maxnet2d[[i]][2]))
  ...
  for (i in 1:length(vars1d)) {
    coef.vec[i] <- sum(unlist(lapply(X = beta.names, FUN = function(x, 
                                                                    y) {
      y %in% x
    }, y = vars1d[i])))
  }
