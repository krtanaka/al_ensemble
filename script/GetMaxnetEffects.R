GetMaxnetEffects <- function (
  ...
  xfacs <- names(model$samplemeans)[!names(model$samplemeans) %in% xvars]
  xvars2d <- NULL
  ...
  if (length(maxnet2d) > 0) {
    ...
    xvars <- xvars[!xvars %in% unlist(maxnet2d)]
  }
  ...
  if (ntot == 0) {
    ...
    return(list(data.frame(NA, effect = ifelse(scale == "log", log(0.01), 0))))
  }
  
  if (is.null(nice.names)) {
    xvar2dnames <- unlist(strsplit(xvars2d, "[*]"))
    nice.names <- data.frame(var = c(xvar2dnames, xvars, xfacs), name = c(xvar2dnames, xvars, xfacs), stringsAsFactors = FALSE)
  }
  ...
  if (length(xvars2d) > 0) {
    for (i in 1:length(xvars2d)) {
      ...
      if (x.name %in% names(model$varmax)) {
        effect.x <- ent + as.vector(maxnet::response.plot(model, v = x.name, plot = FALSE, type = "link")$pred)
      } else {
        effect.x <- log(0.01)
      }
      
      if (y.name %in% names(model$varmax)) {
        effect.y <- ent + as.vector(maxnet::response.plot(model, v = y.name, plot = FALSE, type = "link")$pred)
      } else {
        effect.y <- log(0.01)
      }
  ...
  if (length(xvars) > 0) {
    for (i in 1:length(xvars)) {
      ...
      suppressWarnings(dat$effect <- ent + as.vector(maxnet::response.plot(model, v = xvars[i], plot = FALSE, type = "link")$pred))
      ...
      if (!is.null(cv.models)) {
        cv.dat <- matrix(data = NA, nrow = 100, ncol = length(cv.models))
        for (f in 1:length(cv.models)) {
          if (is.list(cv.models[[f]])) {
            suppressWarnings(cv.dat[, f] <- maxnet::response.plot(cv.models[[f]], type = "link", v = xvars[i], plot = FALSE)$pred + cv.models[[f]]$entropy * as.integer(add.entropy))
          } else {
            cv.dat[, f] <- NA
          }
        }
        ...
        uppers <- apply(X = cv.dat, MARGIN = 1, FUN = "quantile", probs = 0.95, na.rm = TRUE)
        lowers <- apply(X = cv.dat, MARGIN = 1, FUN = "quantile", probs = 0.05, na.rm = TRUE)
        cv.var <- apply(X = cv.dat, MARGIN = 1, FUN = var, na.rm = TRUE)
  ...
  if (length(xfacs) > 0) {
    for (i in 1:length(xfacs)) {
      ...
      names(dat) <- "x"
      suppressWarnings(dat$effect <- ent + as.vector(maxnet::response.plot(model, v = xfacs[i], plot = FALSE, type = "link")$pred))
      ...
      if (!is.null(cv.models)) {
        ...
        for (f in 1:length(cv.models)) {
          if (is.list(cv.models[[f]])) {
            cv.dat[, f] <- maxnet::response.plot(cv.models[[f]], type = "link", v = xfacs[i], plot = FALSE, levels = dat$x)$pred + ent
          } else {
            cv.dat[, f] <- NA
          }
        }
