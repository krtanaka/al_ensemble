MakeEnsembleXtable = function (model.names = c("maxnet", "cloglog", "hpoisson", "poisson", "negbin"), 
                               ensemble = T, weights = NULL, converge.vec = NULL, 
                               abund.check.vec = NULL, cor.method = "pearson", preds.table = NULL, 
                               scale.facs = NULL, efh.breaks = NULL, areas = NULL, filename) 
{
  dim.names <- "Model"
  ...
  if (sum(c(is.null(preds.table) == F, is.na(preds.table) == F)) > 1) {
    ...
    for (m in 1:length(model.names)) {
      ...
      if (model.names[m] %in% preds.table$Model) {
        ...
        pred.cvpred <- ifelse(RMSE(pred = pred.dat$pred, obs = pred.dat$abund) < RMSE(pred = pred.dat$cvpred, obs = pred.dat$abund), "cvpred", "pred")
        prob.cvprob <- ifelse(pred.cvpred == "cvpred", "cvprob", "prob")
        N.vec[m] <- sum(pred.dat$abund > 0, na.rm = TRUE)
    ...
    digs <- ifelse(min(rmse.vec, na.rm = TRUE) > 1000, 0, ifelse(min(rmse.vec, na.rm = TRUE) < 10, 2, 1))
  ...
  if (is.logical(converge.vec)) {
    ...
    if (ensemble) {
      etable[n.models + 3, converge.col] <- TRUE
    }
  }
  ...
  if (is.logical(abund.check.vec)) {
    ...
    if (ensemble) {
      etable[n.models + 3, abund.check.col] <- TRUE
    }
  }
  ...
  if (is.numeric(scale.facs)) {
    ...
    for (m in 1:length(model.names)) {
      ...
      if (model.names[m] %in% names(scale.facs)) {
        scale.fac.vec[m] <- scale.facs[which(names(scale.facs) == model.names[m])]
      }
    }
  ...
  if (is.numeric(weights)) {
    ...
    for (m in 1:length(model.names)) {
      ...
      if (model.names[m] %in% names(weights)) {
        weight.vec[m] <- weights[which(names(weights) == model.names[m])]
      }
    }
    
    etable[model.rows, w.col] <- format(round(weight.vec, 
                                              2), nsmall = 2)
    if (ensemble) {
      etable[n.models + 3, w.col] <- 1
    }
  }
  ...
  if (is.numeric(efh.breaks)) {
    ...
    for (m in 1:length(model.names)) {
      ...
      if (model.names[m] %in% names(efh.breaks)) {
        breaks.vec[m] <- efh.breaks[which(names(efh.breaks) == model.names[m])]
      }
    }
    ...
    if (ensemble) {
      ...
      # etable[n.models + 3, break.col] <- format(round(efh.breaks[which(names(efh.breaks) == "ensemble")], 2), nsmall = 2)
      etable[n.models + 3, break.col] <- format(round(mean(efh.breaks[which(names(efh.breaks) == "ensemble")]), 2), nsmall = 2)
  ...
  if (is.numeric(areas)) {
    ...
    for (m in 1:length(model.names)) {
      ...
      if (model.names[m] %in% names(areas)) {
        area.vec[m] <- areas[which(names(areas) == model.names[m])]
      }
    }
    
    etable[model.rows, area.col] <- format(round(area.vec, 
                                                 -2), big.mark = ",")
    if (ensemble) {
      
      # etable[n.models + 3, area.col] <- format(round(areas[which(names(areas) == "ensemble")], -2), big.mark = ",")
      etable[n.models + 3, area.col] <- format(round(mean(areas[which(names(areas) == "ensemble")], -2)), big.mark = ",")
  ...
  xtable::print.xtable(ensemble.table, type = "html", file = filename, 
                       ...
                       html.table.attributes = 2, include.colnames = getOption("xtable.include.colnames", FALSE), 
