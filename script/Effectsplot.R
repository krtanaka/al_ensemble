Effectsplot = function (effects.list, 
                        region = NA, 
                        crs = NA, 
                        nice.names = NULL, 
                        vars = "all") {
  
  if (length(vars) > 1 && all(vars != "all", na.rm = TRUE) && all(is.character(vars))) {
  ...
  for (j in 1:length(effects.list2)) {
    ...
    if (nrow(e.data) == 1600) {
      ...
      if (xname %in% c("lon", "Longitude", "long")) {
        ...
        label.spots <- data.frame(
          con.data2[round(nrow(con.data2) / 2), 1:2], 
        ...
        for (i in 2:length(con.data)) {
          c.dat <- data.frame(
            lon = con.data[[i]]$x, 
            lat = con.data[[i]]$y, 
            effect = con.data[[i]]$level, 
            group = i
          )
          
          label.spots <- rbind(
            label.spots, 
            data.frame(c.dat[round(nrow(c.dat) / 2), 1:2], tag = con.data[[i]]$level)
          )
        ...
        MAP <- akgfmaps::get_base_layers(
          select.region = tolower(region), 
          set.crs = "auto"
        )
        ...
        spot.data2 <- data.frame(
          sf::st_coordinates(spots.sf2), 
          label = spots.sf2$tag
        )
        
        e.data2 <- data.frame(
          sf::st_coordinates(e.ef2), 
          e.ef2$effect, 
          group = e.ef2$group
        )
        ...
        if (tolower(region) == "goa") {
          ...
          MAP$lon.breaks <- c(-170, -160, -150, -140, 
                              -130)
        }
        ...
        var.plot <- ggplot2::ggplot() + 
          ...
          ggplot2::coord_sf(
            xlim = MAP$plot.boundary$x + ext.adjust.x, 
        ...
        var.plot <- ggplot2::ggplot() + 
          ...
          ggplot2::geom_label(
            data = label.spots, 
            ggplot2::aes(x = x, y = y, label = round(tag, 1)), 
            fill = "white", 
            label.size = NA
          ) + 
          ggplot2::xlab(xname) + 
    ...
    if (nrow(e.data) == 100) {
      ...
      y.lim <- c(
        ifelse(min(e.data$lower, na.rm = TRUE) < lower.lim, lower.lim, NA), 
    ...
    if (nrow(e.data) < 10) {
      ...
      var.plot <- ggplot2::ggplot() + 
        ggplot2::geom_segment(
          data = e.data, 
          ggplot2::aes(y = effect, yend = effect, x = x - 0.35, xend = x + 0.35), 
          size = 2
        ) + 
        ggplot2::geom_segment(
          data = e.data, 
          ggplot2::aes(y = lower, yend = lower, x = x - 0.35, xend = x + 0.35), 
          size = 1, 
          linetype = 2
        ) + 
        ggplot2::geom_segment(
          data = e.data, 
          ggplot2::aes(y = upper, yend = upper, x = x - 0.35, xend = x + 0.35), 
          size = 1, 
          linetype = 2
        ) + 
        ggplot2::xlab(xname) + 
