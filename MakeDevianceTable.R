MakeDevianceTable = function (model.names = NULL, model.types = NULL, nice.names = NULL, dev.list, model.weights = NULL, filename) {
  
  if (is.null(model.weights) || all(is.na(model.weights))) {
    good.models <- 1:length(dev.list)
  } else {
    good.models <- which(model.weights > 0)
  }
  
  if (is.null(model.names) || all(is.na(model.names))) {
  ...
  if (is.null(model.types) == F && all(is.na(model.types) == F)) {
    t.col <- 2
    columns <- c(columns, "Type")
  }
  
  if (is.null(model.weights) == F && all(is.na(model.weights) == F)) {
  ...
  for (d in 1:length(good.models)) {
    ...
    if ("Weight" %in% columns) {
      dtable[1 + d, w.col] <- round(model.weights[good.models[d]], 
                                    3)
    }
    ...
    for (t in 1:n.terms) {
      ...
      if (length(d.term > 0)) {
        dtable[(1 + d), term.col[t]] <- round(model.dev[d.term], 
                                              1)
      } else {
        dtable[(1 + d), term.col[t]] <- "--"
      }
      
      if (is.na(dtable[(1 + d), term.col[t]])) {
        dtable[(1 + d), term.col[t]] <- "--"
      }
    }
  }
  ...
  mean.matrix <- matrix(nrow = length(model.rows), ncol = length(term.col), 
                        data = as.numeric(dtable[model.rows, term.col]), byrow = F)
  
  term.means <- round(apply(mean.matrix, MARGIN = 2, FUN = mean, 
                            na.rm = T), 1)
  ...
  if ("Weight" %in% columns) {
    ...
    term.means2 <- round(apply(mean.matrix * model.weights[good.models], 
                               MARGIN = 2, FUN = sum, na.rm = T), 1)
  ...
  xtable::print.xtable(deviance.table, 
                       type = "html", 
                       file = filename, 
                       ...
                       hline.after = getOption("xtable.hline.after", c(-1, 1, nrow(deviance.table))))
  ...
  xtable::print.xtable(deviance.table2, type = "html", 
                       file = paste0(strsplit(filename, split = ".html")[[1]], "2.html"), 
                       include.rownames = getOption("xtable.include.rownames", FALSE),
                       html.table.attributes = 2, 
                       include.colnames = getOption("xtable.include.colnames", FALSE), 
                       hline.after = getOption("xtable.hline.after", c(-1, 1, nrow(deviance.table2))))
}