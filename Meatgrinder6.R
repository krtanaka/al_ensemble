...
nice.names<-data.frame(stringsAsFactors = F,
                       ...
                       name2=c("longitude","latitude","bottom depth","slope","aspect east","aspect north","curvature",
...
nice.names2<-data.frame(var=nice.names$var,name=nice.names$name,stringsAsFactors = F)
...
while(done==F){
  ...
  if(is.na(species.vec)){species.vec<-unique(masterplan$Abbreviation)}
  if(is.na(region.vec)){region.vec<-c("AI","EBS","GOA")}
  
  unclaimed<-which(is.na(masterplan$Claimed) & masterplan$Region%in%region.vec & masterplan$Abbreviation%in%species.vec)
  ...
  if(length(unclaimed)==0 & rerun==F){
    ...
    if(n.pres>=50){
      ...
      if(cloglog.abund.check==F){
        ...
        try(cloglog.model<-FitGAM(gam.formula = alt.gam.formula,data = species.data,
      ...
      for(m in 1:length(model.vec)){
        ...
        if(model.weights[m]>0){
          ...
          print(MakeAKGFDensityplot(region = tolower(region),density.map = cv.raster,buffer = .98,
                                    col.palette = "magma",col.palette.limits = c(.1,.9),
                                    survey.area = raster.stack$bdepth,legend.title = "CV of Abundance",
                                    title.name = paste0(figure.name,"\n(",model.name,")")))
          grDevices::dev.off()
          
          # Validation plot
          grDevices::png(filename = paste0(species.path,"/",model.name,"_residuals.png"),width = 6,height = 6,res=600,units="in")
          MakeCrossValidationPlots(error.data = model.errors[[m]],method = "spearman",make.hist = F)
          grDevices::dev.off()
          
          # Abundance plot
          grDevices::png(filename = paste0(species.path,"/",model.name,"_abundance.png"),width = png.width,height = png.height,res=600,units="in")
          print(MakeAKGFDensityplot(region = tolower(region),density.map = abund.list[[m]],buffer = .98,
                                    survey.area = raster.stack$bdepth,legend.title = "Abundance",
                                    title.name = paste0(figure.name,"\n(",model.name,")")))
          grDevices::dev.off()
          
          # EFH plot
          grDevices::png(filename = paste0(species.path,"/",model.name,"_efh.png"),width = png.width,height = png.height,res=600,units="in")
          print(MakeAKGFEFHplot(region=tolower(region),efh.map = efh.list[[m]],survey.area = raster.stack$bdepth,
      ...
      grDevices::png(filename = paste0(species.path,"/ensemble_residuals.png"),width = 6,height = 6,res=600,units="in")
      ...
      # Ensemble abundance
      grDevices::png(filename = paste0(species.path,"/ensemble_abundance.png"),width = png.width,height = png.height,res=600,units="in")
      print(MakeAKGFDensityplot(region = tolower(region),density.map = ensemble.abund,buffer = .98,
                                survey.area = raster.stack$bdepth,legend.title = "Abundance",
                                title.name = figure.name))
      grDevices::dev.off()
