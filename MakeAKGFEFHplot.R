MakeAKGFEFHplot = function (
    region, efh.map, drop = 10^8, survey.area = "default", 
    ext.adjust = "default", legend.pos = "default", legend.labels = c("95%", "75%", "50%", "25%"), 
    legend.title = NA, col.palette = "viridis", col.palette.limits = c(0, 1), 
    title.name = NA, title.pos = "default"
) {
  ...
  efh.vals2 <- is.na(efh.vals) == FALSE
  ...
  outside <- order(areas, decreasing = TRUE)[1]
  ...
  efhplot <- ggplot2::ggplot() + 
    ...
    ggplot2::geom_sf(data = efhdummy3, fill = NA, size = 0.3) + 
    ...
    viridis::scale_fill_viridis(discrete = TRUE, name = legend.title, labels = legend.labels) + 
    ggplot2::theme_bw() + 
  ...
  if (!is.na(title.name) && length(title.pos) == 2) {
    efhplot <- efhplot + ggplot2::geom_label(data = data.frame(x = title.pos[1], y = title.pos[2], label = title.name), ggplot2::aes(x = x, y = y, label = label, hjust = 0, vjust = 1), size = 5)
  }
  
  if (length(legend.pos) == 2) {
    efhplot <- efhplot + ggplot2::guides(color = ggplot2::guide_legend(override.aes = list(size = 4, shape = 15)))
  }
