...
nice.names <- data.frame(
  ...
  var = c(
    "lon", "lat", "depth", "sst", "chla", "dhw"
  ),
  
  name = c(
    "Longitude", "Latitude", "Depth (m)", "Sea_Surface_Temperature (degC)", "Chlorophyll_A (mg-3)", "Degree_Heating_Weeks (weeks)"
  ),
  
  name2 = c(
    "longitude", "latitude", "depth", "sst", "chla", "dhw"
  )
)
...
bathy <- raster("Variables/pacific/tutuila/Processed_Bathy.nc")
sst <- raster("Variables/pacific/tutuila/Processed_SST.nc")
chla <- raster("Variables/pacific/tutuila/Processed_Chla.nc")
dhw <- raster("Variables/pacific/tutuila/Processed_DHW.nc")
...
if(negbin.converge & negbin.abund.check){
  ...
  source("GetGAMEffects.R")
  negbin.effects <- GetGAMEffects(model = negbin.model,
                                  data = species.data,vars = "all",
                                  scale = "log",
...
for(m in 1:length(model.vec)){
  ...
  if(model.weights[m] > 0){
    ...
    raster::writeRaster(x = var.list[[m]], 
                        filename = paste0(species.path,"/", model.name,"_abund_variance"), 
                        overwrite = T)
